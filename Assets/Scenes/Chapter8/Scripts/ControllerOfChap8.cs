...
public class ControllerOfChap8 : MonoBehaviour {
    ...
    public List<float> PreSONZAI = new List<float>();
    public List<float> SONZAI = new List<float>();
    ...
    private void ExecutePythonSouce()
    {
        ...
        var ResultPreSONZAI = scriptScope.GetVariable<IronPython.Runtime.List>("preSONZAI");
        var tmpPreSONZAI = ResultPreSONZAI.Cast<double>().ToList();
        PreSONZAI = tmpPreSONZAI.ConvertAll(x => (float)(double)x);
        var ResultSONZAI = scriptScope.GetVariable<IronPython.Runtime.List>("SONZAI");
        var tmpSONZAI = ResultSONZAI.Cast<double>().ToList();
        SONZAI = tmpSONZAI.ConvertAll(x => (float)(double)x);
